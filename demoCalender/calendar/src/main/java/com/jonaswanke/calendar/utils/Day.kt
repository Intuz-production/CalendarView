...
class Day(
    ...
    _dayMonth: Int = TODAY.get(Calendar.DAY_OF_MONTH),
    _month: Int = TODAY.get(Calendar.MONTH)
) : Comparable<Day> {
    constructor(week: Week, day: Int) : this(week.year, week.week, day)
    constructor(cal: GregorianCalendar)
            : this(cal.get(Calendar.YEAR), cal.get(Calendar.WEEK_OF_YEAR), cal.get(Calendar.DAY_OF_WEEK), cal.get(Calendar.DAY_OF_MONTH), cal.get(Calendar.MONTH))

    private val cal: GregorianCalendar = GregorianCalendar().apply {
        set(Calendar.YEAR, _year)
        set(Calendar.MONTH, _month)
        set(Calendar.DAY_OF_MONTH, _dayMonth)
        timeOfDay = 0
    }
    val year = cal.get(Calendar.YEAR)
    val week = cal.get(Calendar.WEEK_OF_YEAR)
    val day = cal.get(Calendar.DAY_OF_WEEK)
    var dayMonth = cal.get(Calendar.DAY_OF_MONTH)
    ...
    fun toCalendar(): GregorianCalendar = GregorianCalendar().apply { timeInMillis = cal.timeInMillis }
//    fun toCalendar(): Calendar = Calendar.getInstance().apply { timeInMillis = cal.timeInMillis }
    ...
    override fun toString() = "$year-$month-$dayMonth-$week-$day"
}

fun GregorianCalendar.toDay() = Day(this)
