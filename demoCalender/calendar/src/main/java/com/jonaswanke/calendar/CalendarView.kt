...
class CalendarView @JvmOverloads constructor(
    ...
    // Pageが横スクロールされた時に発声するイベント
    var onScrolledPageListener: (() -> Unit)? = null
    ...
    var visibleStart: Day
        ...
        set(value) {
            ...
            // pageModelsの範囲外の場合、RangeHeaderVIewが表示されなくなるので、
            // ここで作り直し
            onRangeUpdated()
        }
    var visibleEnd: Day
...
//        pagerAdapter = object : InfinitePagerAdapter<Day, RangeView>(visibleStart, 6) {
        pagerAdapter = object : InfinitePagerAdapter<Day, RangeView>(visibleStart, 2) {
            ...
            override fun instantiateItem(indicator: Day, oldView: RangeView?): RangeView {
                if (BuildConfig.DEBUG)
                    Log.d("TAG", "CalendarView InfinitePagerAdapter instantiateItem start. indicator $indicator")
                ...
                if (BuildConfig.DEBUG)
                    Log.d("TAG", "CalendarView InfinitePagerAdapter instantiateItem before getEventsForRange. eventsSize:${view.events.size}")
                // Set/update events
                if (oldView == null) {
                ...
                if (BuildConfig.DEBUG)
                    Log.d("TAG", "CalendarView InfinitePagerAdapter instantiateItem after getEventsForRange. eventsSize:${view.events.size}")
                ...
                launch(UI) {
                    ...
                    while (weeksLeft > 0) {
                        if (BuildConfig.DEBUG)
                            Log.d("TAG", "CalendarView InfinitePagerAdapter instantiateItem launch(UI) . week:${week.start}")
        ...
        pager.listener = object : InfiniteViewPager.OnInfinitePageChangeListener {
            ...
            override fun onPageScrollStateChanged(state: Int) {
                if (state == ViewPager.SCROLL_STATE_IDLE) {
                    ...
                    // Pageが横スクロールされた時にイベントを発生させて、呼び出し元に通知する
                    // 上記、onPageScrolled だと何度も発生してしまう。
                    // その為、stateが落ち着いたこの時に、発生させることとする
                    onScrolledPageListener?.invoke()
                }
            }
        }
    ...
    private fun getEventsForRange(range: DayRange): List<Event> {
        if (BuildConfig.DEBUG)
            Log.d("TAG", "CalendarView getEventsForRange start. rangeStart:${range.start} rangeEnd:${range.endInclusive} eventsKeysSize:${events.keys.size}")
        var currentWeek = events.keys.minBy { it.start } ?: return emptyList()
        val viewEvents = mutableListOf<Event>()

        if (BuildConfig.DEBUG)
            Log.d("TAG", "CalendarView getEventsForRange processing. events:${events}")

        while (currentWeek.start < range.endExclusive.start) {
            if (BuildConfig.DEBUG)
                Log.d("TAG", "CalendarView getEventsForRange processing. currentWeek:${currentWeek} currentWeekStart:${currentWeek.start} currentWeekEnd:${currentWeek.end}")
            if (BuildConfig.DEBUG)
                Log.d("TAG", "CalendarView getEventsForRange processing. currentWeekEvent:${events[currentWeek]}")
        ...
        if (BuildConfig.DEBUG)
            Log.d("TAG", "CalendarView getEventsForRange end. rangeStart:${range.start.start} rangeEnd:${range.endExclusive.start} viewEventsSize:${viewEvents.size}")
        return viewEvents
    }
    ...
    fun setEventsForWeek(week: Week, events: List<Event>) {
        ...
        if (BuildConfig.DEBUG)
            Log.d("TAG", "CalendarView setEventsForWeek start. week:$week")

        this.events[week] = events

        if (BuildConfig.DEBUG)
            Log.d("TAG", "CalendarView setEventsForWeek eventsAdded. eventsSize:${events.size}")

        when (range) {
            RANGE_DAY -> {
                views.forEach {
                    val view = views[it.key]
                        if (view != null)
                        view.events = getEventsForRange(view.range)
                }
            }
    ...
    fun scrollOnTime(){
        ...
        val posY = hours.hourHeight * time

        scrollPosition = posY.toInt()
    }
}
